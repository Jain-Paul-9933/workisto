...
| F4 | **Place search on the map** ✅ | Nobody knows their coordinates — they know "Koramangala, Bengaluru". The shared map picker now leads with a debounced, keyboard-navigable place search; lat/lng became something the app *shows back* (reverse-geocoded "Selected: …") rather than something anyone types, with manual entry demoted to a fallback toggle. Geocoding is proxied **server-side** (`app/api/geocode`): Nominatim's policy wants an identifying User-Agent and ~1 req/sec, which only a server can honour centrally — so requests queue behind a shared throttle and results are cached, and the route requires a session so it isn't an open geocoding proxy. Results carry a bounding box, so `fitBounds` zooms a city out and a street in. Both provider onboarding and customer search inherit it. **Verified against the live stack**: 401 unauthenticated, real bbox-bearing results, sub-3-character queries short-circuited, reverse lookup naming a seeded pin. |

Supporting work: a `seed_demo` management command builds a ready-to-explore
marketplace (categories, five providers incl. one rural, customers, completed
bookings and reviews so ratings and search ranking have something to chew on).
Idempotent — safe to re-run. See the README's *Demo data* section.
